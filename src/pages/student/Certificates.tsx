          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative w-full sm:w-[360px]">
              ...
              <input
                ...
                className="
                  w-full bg-white dark:bg-[#091a2c] border border-gray-200 dark:border-white/10 rounded-2xl py-4 pl-12 pr-4
                  text-[#080E2F] dark:text-white placeholder:text-gray-400 outline-none focus:border-blue-500 transition
                "
              />
            </div>
            ...
            <div className="hidden sm:flex bg-white dark:bg-[#091a2c] border border-gray-200 dark:border-white/10 rounded-2xl p-1">
              <button
                ...
                className={`w-12 h-12 rounded-xl flex items-center justify-center transition
                  ${viewMode === "grid" ? "bg-blue-500/15 text-blue-600 dark:text-blue-400" : "text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5"}
                `}
              >
                <Grid3X3 size={22} />
              </button>
              ...
              <button
                ...
                className={`w-12 h-12 rounded-xl flex items-center justify-center transition
                  ${viewMode === "list" ? "bg-blue-500/15 text-blue-600 dark:text-blue-400" : "text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5"}
                `}
              >
                <List size={24} />
              </button>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-[320px_1fr] gap-6">
              ...
              <div className="flex flex-col justify-center ">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5 border-y border-gray-200 dark:border-white/10 py-5 mt-5">
                  <InfoItem icon={Calendar} title="Data de Conclusão:" value={featuredCertificate.conclusionDate} />
                  <InfoItem icon={Award} title="Cód. Validação:" value={featuredCertificate.id} />
                  <InfoItem icon={ShieldCheck} title="Validade: 1 ano" value={`Válido até ${featuredCertificate.validUntil}`} success />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-5">
                  ...
                  <a
                    ...
                    className="border border-slate-300 dark:border-white/20 text-slate-700 dark:text-gray-300 rounded-2xl py-4 font-semibold flex items-center justify-center gap-3 hover:bg-slate-50 dark:hover:bg-white/5 transition shadow-2xl cursor-pointer"
                  >
          ...
          <section>
            ...
            <div className={viewMode === "grid" ? "grid grid-cols-1 xl:grid-cols-2 gap-6" : "flex flex-col gap-4"}>
              {otherCertificates.map((certificate) => {
                ...
                return (
                  <article key={certificate.dbId} className="bg-white dark:bg-[#091a2c] border border-gray-200 dark:border-white/10 rounded-3xl p-5 sm:p-6 shadow-2xl transition-all duration-300 ease-in-out hover:scale-105 dark:shadow-blue-500 dark:shadow-sm cursor-pointer">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 ">
                      <div className="flex gap-4">
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5">
                            <SmallInfo icon={Calendar} title="Data de Conclusão:" value={certificate.conclusionDate} />
                            <SmallInfo icon={ShieldCheck} title="Validade: 1 ano" value={`Válido até ${certificate.validUntil}`} success />
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
                      <button
                        ...
                        className="border border-blue-500/40 text-blue-600 dark:text-blue-400 rounded-2xl py-3 font-semibold flex items-center justify-center gap-3 hover:bg-blue-500/10 transition cursor-pointer"
                      >
                        <Eye size={21} />
                        Ver
                      </button>
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="border border-slate-300 dark:border-white/20 text-slate-700 dark:text-gray-300 rounded-2xl py-3 font-semibold flex items-center justify-center gap-3 hover:bg-slate-50 dark:hover:bg-white/5 transition cursor-pointer"
                      >
    ...
    <div className="bg-white dark:bg-[#091a2c] border border-gray-200 dark:border-white/10 rounded-3xl p-6 shadow-2xl dark:shadow-sm dark:shadow-blue-500 cursor-pointer transition-all duration-300 ease-in-out hover:scale-105">
      <div className="flex items-center gap-5">
        ...
        <div>
          ...
          <h2 className="text-3xl font-bold text-[#080E2F] dark:text-white mt-1">{value}</h2>
...
function CertificatePreview({ studentName, courseTitle, }: { studentName: string; courseTitle: string; }) {
  return (
    <div className="relative w-full overflow-hidden rounded-2xl shadow-2xl border border-gray-200 aspect-[1.414] bg-white">
      {/* Fundo do Canva */}
      <img 
      ...
      {/* Student Name */}
      <div className="absolute top-[42%] left-[58%] -translate-x-1/2 -translate-y-1/2 w-full text-center px-8">
        <p className="text-[#1C2B4B] font-bold text-xl sm:text-2xl">
          {studentName}
        </p>
      </div>

      {/* Course Title */}
      <div className="absolute top-[65%] left-[58%] -translate-x-1/2 -translate-y-1/2 w-full text-center px-8">
        <p className="text-[#444444] text-sm sm:text-md line-clamp-2">
          {courseTitle}
        </p>
      </div>
    </div>
